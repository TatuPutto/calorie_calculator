function updateValuesOnConsumedFoodAddition(foodId, foodAmount, activeMeal, foods, consumedFoods, totalConsumption) {
    ...
    for(var i = 0; i < consumedFoods.length; i++) {
        if(consumedFoods[i].mealName == activeMeal) {
            consumedFoods[i].mealCourses.push(addedFood);
            break;
        }
    }
